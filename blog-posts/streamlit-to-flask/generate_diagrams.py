...
def create_traditional_vs_streamlit():
    ...
    with dot.subgraph(name='cluster_traditional') as c:
        c.attr(label='Traditional Web Development', style='rounded', bgcolor='#f0f4f8', fontname='Arial', fontsize='14')
        c.node('backend', 'Backend\n(Python, Java, etc.)', fillcolor='#4a90a4', fontcolor='white')
        c.node('frontend', 'Frontend\n(HTML, CSS, JS)', fillcolor='#e07b53', fontcolor='white')
        c.edge('backend', 'frontend', dir='both', color='#333333', penwidth='2')

    # Streamlit subgraph
    with dot.subgraph(name='cluster_streamlit') as c:
        c.attr(label='Streamlit', style='rounded', bgcolor='#f0f8f0', fontname='Arial', fontsize='14')
        c.node('python', 'Python\nCode', fillcolor='#4a90a4', fontcolor='white')
        c.node('abstraction', 'Streamlit\nAbstraction', fillcolor='#7c5caf', fontcolor='white')
...
def create_rerun_cycle():
    ...
    dot.node('cached', 'Cached?', shape='diamond', fillcolor='#d9edf7', fontcolor='#31708f')
...
def create_tech_stack_decision():
    ...
    dot.node('seo', 'Need SEO +\ndynamic pages?', shape='diamond', fillcolor='#d9edf7', fontcolor='#31708f')
    dot.node('interactive', 'High\ninteractivity?', shape='diamond', fillcolor='#d9edf7', fontcolor='#31708f')
    dot.node('complex', 'Complex features?\n(users, auth)', shape='diamond', fillcolor='#d9edf7', fontcolor='#31708f')

    # Outcome nodes
    dot.node('streamlit', 'Stay with\nStreamlit/Dash', fillcolor='#5cb85c', fontcolor='white')
